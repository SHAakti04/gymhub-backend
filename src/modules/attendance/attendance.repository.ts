...
export const attendanceRepository = {
  async getDailyQr(gymId: string, date = istDateKey()) {
    const rows = await query<RowDataPacket[]>(
      "SELECT * FROM daily_qr_codes WHERE gym_id = ? AND qr_date = ? LIMIT 1",
      [gymId, date],
    );
    return rows[0] ?? null;
  },
  ...
  async upsertDailyQr(input: { gymId: string; code: string; generatedByUserId: string | null }) {
    ...
    if (existing) {
      await query<ResultSetHeader>(
        "UPDATE daily_qr_codes SET code = ?, generated_by_user_id = ?, created_at = NOW() WHERE id = ?",
        [input.code, input.generatedByUserId, existing.id],
      );
      return this.getDailyQr(input.gymId);
    }

    await query<ResultSetHeader>(
      "INSERT INTO daily_qr_codes (id, gym_id, qr_date, code, generated_by_user_id) VALUES (?, ?, ?, ?, ?)",
  ...
  async getTodayAttendance(gymId: string) {
    return query<RowDataPacket[]>(
      "SELECT * FROM attendance_sessions WHERE gym_id = ? AND attendance_date = ? ORDER BY created_at DESC",
      [gymId, istDateKey()],
    );
  },
  ...
  async getOpenSession(memberId: string) {
    const rows = await query<RowDataPacket[]>(
      "SELECT * FROM attendance_sessions WHERE member_id = ? AND attendance_date = ? LIMIT 1",
      [memberId, istDateKey()],
    );
    return rows[0] ?? null;
  },
  ...
  async completeCheckOut(memberId: string) {
    await query<ResultSetHeader>(
      "UPDATE attendance_sessions SET check_out_time = ? WHERE member_id = ? AND attendance_date = ?",
  ...
  async getAbsentToday(gymId: string) {
    return query<RowDataPacket[]>(
      ...
      LEFT JOIN attendance_sessions a
        ON a.member_id = m.id AND a.attendance_date = ?
      WHERE m.gym_id = ?
        AND m.status = 'active'
        AND a.id IS NULL
  ...
  async queueAbsenceReminders(gymId: string, date = istDateKey()) {
    ...
    for (const [index, member] of absent.entries()) {
      ...
      const result = await query<ResultSetHeader>(
        `
        INSERT IGNORE INTO absence_reminders
      ...
      await query<ResultSetHeader>(
        ...
        VALUES (?, ?, NULL, ?, ?, 'daily-absence-motivation', 'whatsapp', 1, 'queued')
        `,
        [
          makeId(),
          gymId,
          phone,
          `Hi ${member.full_name}, we missed you at the gym today. ${motivation}`,
        ],
      );
  ...
  async listAbsenceReminders(gymId: string, date = istDateKey()) {
    return query<RowDataPacket[]>(
      ...
      JOIN users u ON u.id = m.user_id
      WHERE ar.gym_id = ? AND ar.reminder_date = ?
